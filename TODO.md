...
### Backend
- [x] 1. Fix `backend/src/middleware/auth.js` — replace `userProfile` with `profile` in `protect` & `optionalAuth` (match Prisma schema relation)
- [x] 2. Create `backend/.env.example` — document all Google/Facebook OAuth + app env keys
- [x] 3. Enhance `backend/src/config/passport.js` — update avatar for existing users, add first/last name mapping, better logging
- [x] 4. Add graceful "strategy not configured" guards in `backend/src/routes/authRoutes.js`
